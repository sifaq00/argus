...
/**
 * ARGUS — JSON fetch over Node's https client.
 *
 * Some upstreams ARGUS depends on cannot be reached with the bundled undici
...
export const ARGUS_UA = 'ARGUS-OSINT/1.0 (+https://github.com/simplifaisoul/argus)';
...
function request(url: string, { timeoutMs = 20000, headers = {} }: RequestOptions): Promise<RawResponse> {
  return new Promise<RawResponse>((resolve, reject) => {
    const req = https.get(
      url,
      {
        headers: { 'User-Agent': ARGUS_UA, Accept: 'application/json', 'Accept-Language': 'en', ...headers },
        timeout: timeoutMs,
      },
      (res) => {
