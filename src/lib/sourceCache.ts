    ...
    const inflight = (async () => {
      try {
        ...
        if (entry?.data.length) {
          console.warn(`[ARGUS] ${key} refresh failed — serving ${entry.data.length} cached cameras`);
        ...
        console.warn(`[ARGUS] ${key} fetch failed with no cache to fall back on:`, e);
