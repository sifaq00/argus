  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (res.status >= 400 && res.status < 500) {
        console.warn(`[ARGUS] ${label} returned ${res.status} — absent from this refresh, not retried`);
        return null;
      }
      if (attempt === 2) console.warn(`[ARGUS] ${label} returned ${res.status} — absent from this refresh`);
    } catch (e) {
      if (attempt === 2) console.warn(`[ARGUS] ${label} failed — absent from this refresh:`, e instanceof Error ? e.message : e);
    }
  }
  return null;
}
...
async function fetchEuropeCameras(): Promise<any[]> {
  ...
  try {
    ...
    console.warn('[ARGUS] Netherlands cameras failed — absent from this refresh:', e instanceof Error ? e.message : e);
  }
...
function withBudget(region: string, fetcher: RegionFetcher): ReturnType<RegionFetcher> {
  ...
  return Promise.race([
    ...
    new Promise<Awaited<ReturnType<RegionFetcher>>>(resolve => {
      timer = setTimeout(() => {
        console.warn(`[ARGUS] cctv:${region} over ${REGION_BUDGET_MS}ms — returning without it`);
        resolve([]);
      }, REGION_BUDGET_MS);
    }),
  ]);
