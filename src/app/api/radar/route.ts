...
/**
 * ARGUS — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'ARGUS/4.2', 'Accept': 'application/json' },
    });

    console.log('[ARGUS] IODA response status:', res.status);
    ...
    console.error('[ARGUS] IODA fetch error:', error);
