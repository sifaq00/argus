...
/**
 * ARGUS — Live Conflict Zone Intelligence API
 * 
  ...
  try {
    ...
    const feedPromises = RSS_FEEDS.map(async (url) => {
      try {
        ...
        if (!res.ok) {
          console.log(`[ARGUS] RSS Fetch Failed for ${url}: ${res.status}`);
          return [];
        }
        
        const xml = await res.text();
        const rawItems = xml.split(/<item>/i).slice(1);
        console.log(`[ARGUS] RSS ${url} returned ${rawItems.length} items`);
        ...
        console.log(`[ARGUS] RSS Fetch Error for ${url}:`, (err as Error).message);
        return [];
      }
    });
    ...
    console.log(`[ARGUS] Mapped ${allEvents.length} conflict events from RSS.`);
  } catch (e) {
...
export async function GET() {
  try {
    ...
    console.error('[ARGUS] Conflict API error:', error);
