...
const FALLBACK_ORIGIN = 'https://argusai.live';
  ...
  return (
    <div className="docs-root min-h-screen bg-[var(--bg-void)] text-[var(--text-primary)] antialiased">
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(900px 480px at 12% -8%, rgba(0,230,118,0.07), transparent 65%), radial-gradient(760px 420px at 92% 4%, rgba(0,229,255,0.05), transparent 62%)',
        }}
      />
        ...
        <div className="max-w-[1600px] mx-auto px-4 md:px-8 h-14 flex items-center gap-3">
          <Link href="/" className="flex items-center gap-3 shrink-0 group">
            <Image
              ...
              width={34}
              height={34}
              className="rounded-md object-contain transition-transform group-hover:scale-105 drop-shadow-[0_0_8px_rgba(0,230,118,0.4)]"
            />
          ...
          <a
            href="https://github.com/sifaq00/argus.git"
            target="_blank"
          ...
          <Link
            href="/app"
            className="hidden md:inline-flex items-center gap-1.5 text-[11px] font-mono tracking-[0.15em] uppercase px-3 h-8 rounded-lg border border-[var(--gold-primary)]/30 bg-[var(--gold-primary)]/[0.08] text-[var(--gold-primary)] hover:bg-[var(--gold-primary)]/[0.18] transition-colors self-center my-auto leading-none"
          >
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-4 md:px-8 flex gap-8 xl:gap-12 items-start">
        {/* ── Left sidebar ── */}
        <nav
          className={`${
            navOpen
          ...
          } fixed lg:sticky top-14 left-0 bottom-0 lg:bottom-auto z-[260] lg:z-auto w-64 lg:w-56 shrink-0 lg:self-start lg:h-[calc(100vh-3.5rem)] overflow-y-auto styled-scrollbar bg-[var(--bg-void)] lg:bg-transparent border-r lg:border-r-0 border-white/[0.06] py-6 pr-2 pl-2 lg:pl-0 transition-all duration-200`}
        >
        ...
        <main ref={mainRef} className="flex-1 min-w-0 py-12 lg:py-16 max-w-3xl">
          ...
          <div className="mb-20">
            ...
            <h1 className="text-[38px] md:text-[52px] leading-[1.05] font-bold tracking-[-0.02em] mb-5">
              ...
              <br />
              <span className="bg-gradient-to-r from-[var(--gold-primary)] via-[#69F0AE] to-[var(--cyan-primary)] bg-clip-text text-transparent">
                ARGUS platform
              </span>
            </h1>

            <p className="text-[15px] leading-[1.75] text-[var(--text-secondary)] max-w-[42rem]">
              ARGUS aggregates aviation, maritime, seismic, conflict, cyber, and OSINT feeds onto a single
...
#   "timestamp": "2026-07-29T12:00:00Z" }`}</Pre>
          ...
          <Section id="self-hosting" eyebrow="Guide" title="Self-Hosting">
            <p>ARGUS needs Node 20+ and no database. A local instance is three commands:</p>
            <Pre label="Local development" lang="bash">{`git clone https://github.com/sifaq00/argus.git
cd argus
npm install
...
npm run test:live  # includes tests that hit live upstream feeds`}</Pre>
            <p>
              ...
              always listens on port 3000 internally; <Code>ARGUS_PORT</Code> controls the host port it is published
              on.
            </p>
...
docker compose up -d`}</Pre>
              ...
              {[
                {
                  ...
                  v: 'Points at the separate RECON scanner backend. SCANNER_KEY must equal that backend’s ARGUS_KEY. Leave both empty to disable RECON — /api/scanner then returns 503 by design.',
                },
                {
                  k: 'SDK_INGEST_KEY',
          ...
          <footer className="border-t border-white/[0.06] pt-6 pb-16 flex flex-wrap items-center gap-x-5 gap-y-2 text-[10px] font-mono text-[var(--text-muted)]">
            {[
              { href: 'https://github.com/sifaq00/argus.git', label: 'GitHub' },
              { href: 'https://x.com', label: 'X' },
              { href: 'https://github.com/sifaq00/argus/issues', label: 'Report an issue' },
            ].map(l => (
              <a
                key={l.label}
                href={l.href}
        ...
        <aside className="hidden xl:block w-52 shrink-0 self-start sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto styled-scrollbar py-16 ml-auto">
