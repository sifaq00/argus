...
export default function Dashboard() {
  ...
  useEffect(() => {
    (window as unknown as { argusWatchFlight?: (f: WatchedFlight) => void }).argusWatchFlight = (f) => {
      if (!f?.icao24) return;
  ...
  const [argusTheme, setArgusTheme] = useState<'core'|'ghost'>('core');

  useEffect(() => {
    document.body.className = argusTheme === 'core' ? '' : `theme-${argusTheme}`;
  }, [argusTheme]);
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    } catch (e) { console.warn('[ARGUS] Suppressed error:', e instanceof Error ? e.message : e); } finally { setDossierLoading(false); }
  }, []);
  ...
  // ── Drawing / AOI ──
  // ArgusMap already owns the draw interaction and the polygon rendering;
  ...
  const handleExportGeoJSON = useCallback(() => {
    downloadFile(
      `argus-aoi-${new Date().toISOString().slice(0, 10)}.geojson`,
  ...
  const fetchEndpoint = useCallback(async (
    ...
    try {
      ...
      console.warn('[ARGUS] Suppressed error:', e instanceof Error ? e.message : e);
  ...
  // ── ARGUS SDK — Intelligence Fusion Layer ──
  ...
  return (
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
        ...
        {showSplash && (
          ...
          >
            ...
            <div className="absolute inset-0 pointer-events-none z-[1]" style={{
              backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,230,118,0.015) 2px, rgba(0,230,118,0.015) 4px)',
            ...
            <div className="relative w-40 h-40 mb-8 flex items-center justify-center z-[2]">
              ...
              >
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 rounded-full" style={{ background: 'var(--gold-primary)', boxShadow: '0 0 12px var(--gold-primary), 0 0 24px rgba(0,230,118,0.3)' }} />
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 w-1 h-1 rounded-full" style={{ background: 'rgba(0,230,118,0.5)', boxShadow: '0 0 6px rgba(0,230,118,0.3)' }} />
              </motion.div>
              ...
              <motion.div
                ...
                className="absolute rounded-full"
                style={{ inset: '40px', border: '1px solid rgba(0,230,118,0.25)' }}
              >
              ...
              <motion.div
                ...
                style={{ border: '2px solid var(--gold-primary)', boxShadow: '0 0 20px rgba(0,230,118,0.2), inset 0 0 20px rgba(0,230,118,0.1)' }}
              >
                <Image
                  src="/argus-logo.webp"
                  alt="ARGUS logo"
                ...
                />
                {/* Crosshair lines */}
                <div className="absolute w-[1px] h-full z-0" style={{ background: 'linear-gradient(to bottom, transparent, rgba(0,230,118,0.3), transparent)' }} />
                <div className="absolute w-full h-[1px] z-0" style={{ background: 'linear-gradient(to right, transparent, rgba(0,230,118,0.3), transparent)' }} />
              </motion.div>
              ...
              <motion.div
                ...
                style={{ background: 'conic-gradient(from 0deg, transparent 0deg, rgba(0,230,118,0.15) 40deg, transparent 80deg)' }}
              />
            </div>

            {/* ── ARGUS title — letter-by-letter stagger ── */}
            <div className="flex items-center gap-[2px] mb-3 z-[2]">
              {'ARGUS'.split('').map((letter, i) => (
                <motion.span
                  key={i}
                  ...
                  style={{ color: 'var(--text-heading)', textShadow: '0 0 30px rgba(0,230,118,0.2)' }}
                >
                  {letter}
                </motion.span>
              ))}
            ...
            <div className="w-64 md:w-80 z-[2]">
              {/* Thin progress track */}
              <div className="relative w-full h-[2px] rounded-full overflow-hidden" style={{ background: 'rgba(0,230,118,0.1)' }}>
                <motion.div
                  initial={{ width: '0%' }}
                  animate={{ width: ['0%', '25%', '50%', '78%', '100%'] }}
                  transition={{ duration: 2.2, delay: 0.5, times: [0, 0.25, 0.5, 0.75, 1], ease: 'easeInOut' }}
                  className="absolute inset-y-0 left-0 rounded-full"
                  style={{ background: 'linear-gradient(90deg, var(--gold-primary), var(--cyan-primary), var(--gold-primary))', boxShadow: '0 0 12px rgba(0,230,118,0.4)' }}
                />
              </div>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '64px', right: '24px' }}>
        <div className="flex items-center gap-3.5 w-fit pointer-events-auto">
          <Link href="/" className="flex items-center gap-3.5 group">
            <Image
              ...
              width={48}
              height={48}
              className="w-11 h-11 md:w-12 md:h-12 rounded-md object-contain shrink-0 drop-shadow-[0_0_12px_rgba(0,230,118,0.6)] transition-transform group-hover:scale-105"
              priority
            />
            <div className="flex flex-col items-start gap-0.5">
              <h1 className="text-xl md:text-2xl font-bold tracking-[0.35em] text-[#00E676] font-mono group-hover:text-white transition-colors">ARGUS</h1>
              <span className="text-[10px] md:text-[11px] font-mono tracking-[0.2em] opacity-80 uppercase text-[#00E676]">OPEN SOURCE INTELLIGENCE</span>
            </div>
          </Link>
        </div>
        <div className="flex items-center gap-3 mt-1 pl-[56px] md:pl-[62px] min-w-0 pr-4">
      ...
      {showLayers && !isMobile && <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} theme={argusTheme} setTheme={setArgusTheme} capabilities={capabilities} />}
      ...
      {!isMobile && <div className="absolute right-2 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-[250] pointer-events-auto bg-black/40 backdrop-blur-sm p-1 rounded-full border border-white/5">
        ...
        {/* ── ARGUS AGENT FEED ── */}
        <div className="relative group">
          <button onClick={() => { setShowAgent(!showAgent); setShowArcGIS(false); setShowRemote(false); }} className={`relative w-8 h-8 rounded-full flex items-center justify-center transition-colors focus:outline-none focus-visible:ring-1 focus-visible:ring-white/50 ${showAgent ? 'bg-[var(--gold-primary)]/20' : 'hover:bg-white/10'}`} title="Argus Agent — anomaly narratives" aria-label="Argus Agent" aria-expanded={showAgent}>
            <Newspaper className={`w-4 h-4 ${showAgent ? 'text-[var(--gold-primary)]' : 'text-white/60'}`} />
            {showAgent && (
              <span
                aria-hidden="true"
                className="absolute -right-1 top-1/2 -translate-y-1/2 h-4 w-[2px] rounded-full bg-current text-[var(--gold-primary)]"
          ...
          <span className="absolute right-11 top-1/2 -translate-y-1/2 px-2 py-1 text-[9px] font-mono tracking-wider text-white/80 bg-black/80 backdrop-blur-sm rounded whitespace-nowrap opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity pointer-events-none">AGENT</span>
          <AnimatePresence>
            {showAgent && (
          ...
          <AnimatePresence>
            {showArcGIS && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="absolute right-12 top-1/2 -translate-y-1/2 w-[340px]">
                <div className="glass-panel p-3 max-h-[70vh] overflow-y-auto styled-scrollbar">
                  <ArcGISPanel
                    onImportLayer={(layer) => setArcgisLayers(prev => [...prev.filter(l => l.id !== layer.id), { ...layer, color: layer.color || '#00E676', visible: true, opacity: layer.opacity ?? 0.8 }])}
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#111] border-b border-[var(--border-primary)]">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 rounded-full bg-[#FF4081] animate-argus-pulse" />
                ...
                <div className="px-3 pb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="hud-text text-[10px] text-[var(--text-primary)]">
                      {mobilePanel === 'layers' ? 'LAYERS & STATS' : mobilePanel === 'markets' ? 'MARKETS & INTEL' : mobilePanel === 'intel' ? 'INTEL FEED' : mobilePanel === 'recon' ? 'ARGUS RECON' : mobilePanel === 'remote' ? 'WORLD REMOTE' : 'SEARCH'}
                    </span>
                  ...
                  {mobilePanel === 'layers' && (
                    <>
                      ...
                      <LayerPanel data={data} activeLayers={activeLayers} setActiveLayers={setActiveLayers} isMobile={true} theme={argusTheme} setTheme={setArgusTheme} capabilities={capabilities} />
                      <div className="mt-8">
                  ...
                  {mobilePanel === 'intel' && (
                    <div className="space-y-2">
                      <div className="glass-panel-sm p-2"><AgentFeed /></div>
                      <IntelFeed data={data} onLocate={(lat, lng) => { setFlyToLocation({ lat, lng, ts: Date.now() }); setMobilePanel(null); }} />
                    </div>
                  )}
                  {mobilePanel === 'search' && (
                    <div className="space-y-2">
      ...
      {(regionDossier || dossierLoading) && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute top-16 md:top-20 left-2 right-2 md:left-1/2 md:right-auto md:-translate-x-1/2 z-[300] md:w-[480px] max-h-[65vh] overflow-y-auto styled-scrollbar">
          <div className="glass-panel p-5 argus-glow">
