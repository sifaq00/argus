...
function ArgusMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core', drawnPolygons = [], arcgisLayers = [], drawMode = null, onDrawComplete, onDrawProgress, onDrawCancel, drawCommand = null, onMapCenter, route = null, userLocation = null, followUser = false, onFollowInterrupt, navigating = false, aircraftAirports = {} }: ArgusMapProps) {
  ...
  useEffect(() => {
    ...
    for (const canvasContextAttributes of attributeFallbacks) {
      try {
        ...
        console.warn('[ARGUS] WebGL context rejected, retrying with weaker attributes:', e instanceof Error ? e.message : e);
      }
    }
    if (!map) return;
    ...
    map.on('load', () => {
      ...
      // Create icons — ARGUS Unified Palette
      ...
      // ══ ARGUS SDK — Lattice Intelligence Mesh ══
      ...
      if (process.env.NODE_ENV === 'development') (window as any).__argusMap = map;
    });
    ...
    ['fl-commercial','fl-private','fl-jets','fl-military'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        popup(coords, `<div style="${pStyle}border:1px solid rgba(255,255,255,0.08);">
          ...
          <button onclick="window.argusWatchFlight && window.argusWatchFlight({ icao24: '${idSafe(p.icao24||'')}', callsign: '${idSafe(cs)}' })" style="width:100%;margin-top:8px;padding:6px 12px;background:rgba(0,229,255,0.10);border:1px solid rgba(0,229,255,0.35);color:#7FE9FF;font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:bold;letter-spacing:0.1em;border-radius:4px;cursor:pointer;">+ WATCH THIS AIRCRAFT</button>
    ...
    ['sdk-sea','sdk-sea-glow','sdk-air','sdk-air-glow','sdk-intel','sdk-intel-glow'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        const srcUrl = p.url || SDK_SOURCE_URLS[p.source] || 'https://argusai.live';
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${urlSafe(srcUrl)}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${htmlEsc(p.source || 'ARGUS')}</a></div>
          </div>
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getSource('argus-buildings')) {
          map.addSource('argus-buildings', {
            type: 'vector',
        ...
        if (!map.getLayer('argus-3d-buildings')) {
          map.addLayer({
            id: 'argus-3d-buildings',
            source: 'argus-buildings',
        ...
        // ── DISABLE 3D ──
        if (map.getLayer('argus-3d-buildings')) map.removeLayer('argus-3d-buildings');
      }
    } catch (e) {
      console.warn('[ARGUS] 3D terrain toggle error:', e);
    }
